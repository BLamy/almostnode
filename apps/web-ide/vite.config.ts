...
// Base URL where `vimWasmAssets()` serves the vim.wasm runtime (worker +
...
const ffmpegCoreEsmRoot = resolve(
  resolvePreferredPnpmPackagePath(workspaceRoot, "@ffmpeg/core", "0.12."),
  "dist/esm",
);
...
export default defineConfig(async ({ mode }) => {
  ...
  return {
    ...
    optimizeDeps: {
      ...
      exclude: [
        "@bokuweb/zstd-wasm",
        // Keep @ffmpeg/ffmpeg unbundled so its `new Worker(new URL('./worker.js',
        // import.meta.url))` pattern resolves the worker correctly.
        "@ffmpeg/ffmpeg",
        "buffer",
        "brotli-wasm",
        "bun:sqlite",
