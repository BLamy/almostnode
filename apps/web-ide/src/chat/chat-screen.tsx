...
import type { AgentHarness, ConversationState } from './conversation-types';
import {
  agentSessionRegistry,
  type ActiveAgentSession,
} from './agent-session-registry';
import { createConversationAdapter } from './adapter-factory';
...
interface ChatScreenProps {
  host: WebIDEHost | null;
}

export function ChatScreen({ host }: ChatScreenProps) {
  ...
  const adapterRef = useRef<ReturnType<typeof createConversationAdapter>>(null);
  ...
  useEffect(() => {
    ...
    const adapter = createConversationAdapter(host, session, agentSessionRegistry);
  ...
  const handleSend = useCallback(
    async (text: string) => {
      if (!host) {
        throw new Error('The workbench is still loading.');
      }
      ...
      if (!active || !active.isRunning()) {
        ...
        try {
          ...
          if (!agentSessionRegistry.isLaunching()) {
            ...
            await host.startAgentSession(selectedHarness);
          }
