...
async function waitForWorkbenchTheme(
  page: Page,
  theme: "light" | "dark",
): Promise<void> {
  await page.waitForFunction(
...
async function waitForHostPrompt(
  page: Page,
  statusPrefix = "Exited",
  timeout = 45000,
): Promise<void> {
  await page.waitForFunction(
    (expectedStatusPrefix: string) => {
      const status =
        document.getElementById("webideTerminalStatus")?.textContent?.trim() ||
        "";
      const term = (window as any).__almostnodeWebIDE?.terminal;
      ...
      return (
        status.startsWith(expectedStatusPrefix) && text.trimEnd().endsWith("$")
      );
    },
    statusPrefix,
    { timeout },
...
async function driveInteractiveHostCommand(
  ...
  while (Date.now() < deadline) {
    ...
    const needsDefaultEnter =
      /what style|which color|use css variables|tailwind\.config|would you like|react server components|alias prefix|select a style|pick a color/i.test(
        tail,
      );
    const needsOverwriteAnswer =
      options.answerOverwriteWithNo &&
      !overwriteAnswered &&
  ...
  throw new Error(
    `Timed out waiting for "${command}" to finish in the host terminal.`,
  );
}

test.describe("web-ide host terminal", () => {
  test("keeps scroll affordances and selection highlight visible", async ({
    page,
  }) => {
    ...
    const readTerminalState = () =>
      page.evaluate(() => {
        ...
        return {
          sliderTop: slider
            ? Math.round(parseFloat(getComputedStyle(slider).top || "0"))
            : 0,
          sliderBackground: slider
            ? getComputedStyle(slider).backgroundColor
            : "",
    ...
    await page.waitForFunction((previousFirstRow) => {
      const rows = document.querySelector("#webideTerminal .xterm-rows");
      return (
        (rows?.firstElementChild?.textContent?.trim() || "") !==
        previousFirstRow
      );
    }, initial.firstRow);
    ...
    await page.waitForFunction(() => {
      return (
        (document.querySelector("#webideTerminal .xterm-selection")
          ?.childElementCount || 0) > 0
      );
    });

    const selectionBackground = await page.evaluate(() => {
      const firstSelectionSegment = document.querySelector(
        "#webideTerminal .xterm-selection div",
      );
      return firstSelectionSegment
        ? getComputedStyle(firstSelectionSegment).backgroundColor
        : "";
    });
    expect(selectionBackground).toContain("255, 122, 89");
  });

  test("keeps terminal tabs isolated and runs preview in a dedicated tab", async ({
    page,
  }) => {
    await loadWebIDE(page);
    ...
    await page.waitForFunction(() => {
      return Array.from(
        document.querySelectorAll(".almostnode-terminal-surface__tab"),
      ).some((node) => node.textContent?.includes("Preview"));
    });

    const activeTabLabel = await page
      .locator(".almostnode-terminal-surface__tab.is-active")
      .first()
      .textContent();
    expect(activeTabLabel).toContain("Terminal 1");

    await page.evaluate(async () => {
      await (window as any).__almostnodeWebIDE.executeHostCommand(
        'printf "tab-one\\n"',
      );
    });
    await page.waitForFunction(() => {
    ...
    await expect(
      page.locator(".almostnode-terminal-surface__tab", {
        hasText: "Terminal 2",
      }),
    ).toHaveCount(1);
    ...
    await page
      .locator(".almostnode-terminal-surface__tab", { hasText: "Terminal 1" })
      .click();
    await page.waitForFunction(() => {
      return (
        document.querySelector("#webideTerminal .xterm-rows")?.textContent || ""
    ...
    const firstTabText = await page
      .locator("#webideTerminal .xterm-rows")
      .textContent();
    expect(firstTabText || "").toContain("tab-one");
    expect(firstTabText || "").not.toContain("tab-two");
  });

  test("mounts OpenCode from the regular terminal and restores the shell tab on close", async ({
    page,
  }) => {
    ...
    await page.waitForFunction(
      () => Boolean((window as any).__OPENCODE_BROWSER_TUI__),
      {
        timeout: 180000,
      },
    );
    ...
    await expect(
      page.locator(".almostnode-terminal-surface__tab.is-active"),
    ...
    await page
      .locator(
        ".almostnode-terminal-surface__tab.is-active .almostnode-terminal-surface__tab-close",
      )
      .click();
    await expect(
      page.locator(".almostnode-terminal-surface__tab.is-active"),
  ...
  test("updates the embedded OpenCode session when the workbench theme changes", async ({
    page,
  }) => {
    test.setTimeout(6 * 60 * 1000);
    await loadWebIDE(page);
    ...
    await page.waitForSelector("#webideTerminalStatus", {
      state: "attached",
      timeout: 30000,
    });
    ...
    const readThemeState = () =>
      page.evaluate(() => {
        const host = (window as any).__almostnodeWebIDE;
        const openCode = (window as any).__OPENCODE_BROWSER_TUI__;
        return {
          rootTheme: document.documentElement.dataset.almostnodeTheme ?? "",
          hostTerminalThemeBackground:
            host?.terminal?.options?.theme?.background ?? "",
          openCodeThemeBackground:
            openCode?.term?.options?.theme?.background ?? "",
          openCodeHostTheme:
            document
              .querySelector(".almostnode-opencode-host")
              ?.getAttribute("data-almostnode-theme") ?? "",
        };
      });

    expect(await readThemeState()).toEqual({
      rootTheme: "dark",
      hostTerminalThemeBackground: "#0e1218",
      openCodeThemeBackground: "#0d1117",
      openCodeHostTheme: "dark",
    });

    await page.evaluate(async () => {
      await (window as any).__almostnodeWebIDE.setWorkbenchColorTheme(
        "Islands Light",
      );
    });
    await waitForWorkbenchTheme(page, "light");
    await expect.poll(readThemeState, { timeout: 30000 }).toEqual({
      rootTheme: "light",
      hostTerminalThemeBackground: "#ffffff",
      openCodeThemeBackground: "#ffffff",
      openCodeHostTheme: "light",
    });
  ...
  test("runs shadcn from the seeded Web IDE workspace with debug output and survives a second run", async ({
    page,
  }) => {
    ...
    expect(firstRunOutput).toMatch(
      /Resolving shadcn@latest|Installing \d+ packages|Applying \d+ file changes|Install changes applied/,
    );
    ...
    await expect
      .poll(
        () => {
          return page.evaluate(() => {
            return (window as any).__almostnodeWebIDE.container.vfs.existsSync(
    ...
    expect(
      consoleMessages.some((message) => {
        return (
          message.includes("[almostnode DEBUG]") &&
