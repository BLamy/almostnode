...
(function (global) {
  ...
  function parsePlaywrightSelector(selector) {
    ...
    if (hasTextMatch) {
      ...
      cssSelector = selector.replace(/:has-text\(["'].+?["']\)/, '').trim() || '*';
    }
  ...
  function createLocator(doc, selector, stepTracker, options = {}) {
    const { nthIndex, filterText: optionFilterText, rootElement, customQueryFn } = options;
    ...
    const filterText = optionFilterText !== undefined ? optionFilterText : parsedTextFilter;
    ...
    const locator = {
      ...
      locator(nestedSelector) {
        ...
        if (!targetEl) {
          return createLocator(doc, nestedSelector, stepTracker, { ...options, rootElement: null });
        }
        return createLocator(doc, nestedSelector, stepTracker, { rootElement: targetEl });
      },

      first() {
        return createLocator(doc, selector, stepTracker, { ...options, nthIndex: 0 });
      },

      last() {
        return createLocator(doc, selector, stepTracker, { ...options, nthIndex: -1 });
      },

      nth(index) {
        return createLocator(doc, selector, stepTracker, { ...options, nthIndex: index });
      },
      ...
      async all() {
        ...
        return elements.map((_, index) =>
          createLocator(doc, selector, stepTracker, { ...options, nthIndex: index })
        );
      },
      ...
      filter(filterOptions) {
        ...
        if (hasText !== undefined) {
          return createLocator(doc, selector, stepTracker, { ...options, filterText: hasText });
        }
        return this;
      },
      ...
      click: async () => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'click', selector, status: 'failed',
              error: error.message,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        }
        ...
        if (el.disabled === true) {
          ...
          while (Date.now() - startTime < maxWait) {
            ...
            if (freshEl && !freshEl.disabled) { el = freshEl; break; }
            if (freshEl) el = freshEl;
          }
          const finalEl = getTargetElement();
          if (finalEl) el = finalEl;
          if (el.disabled === true) {
            const error = new Error(`${desc}.click(): element is still disabled after waiting`);
            try {
              stepTracker?.addStep({
                type: 'action', action: 'click', selector, status: 'failed',
                error: error.message,
        ...
        if (cursorAnimationCallback && position) {
          try {
            await cursorAnimationCallback({
              action: 'click', position, selector,
        ...
        try {
          stepTracker?.addStep({
            type: 'action', action: 'click', selector, status: 'passed',
            description: `page.${desc}.click()`, position,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        ...
        if (typeof el.click === 'function') {
          ...
          const clickEvent = new (localDocWindow?.MouseEvent || MouseEvent)('click', {
            bubbles: true, cancelable: true, view: localDocWindow,
          });
      ...
      hover: async () => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'hover', selector, status: 'failed',
              error: error.message, description: `page.${desc}.hover()`,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        }
        ...
        if (cursorAnimationCallback && position) {
          try {
            await cursorAnimationCallback({
              action: 'hover', position, selector,
              description: `page.${desc}.hover()`,
            });
          } catch (e) { console.warn('Cursor animation failed:', e); }
        }

        try {
          stepTracker?.addStep({
            type: 'action', action: 'hover', selector, status: 'passed',
            description: `page.${desc}.hover()`, position,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }

        const localDocWindow = doc.defaultView || doc.parentWindow || window;
        const ME = localDocWindow?.MouseEvent || MouseEvent;
        el.dispatchEvent(new ME('mouseover', { bubbles: true, cancelable: true, view: localDocWindow }));
        el.dispatchEvent(new ME('mouseenter', { bubbles: false, cancelable: false, view: localDocWindow }));
        await delay(0);
      },
      ...
      clear: async () => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'clear', selector, status: 'failed',
              error: error.message, description: `page.${desc}.clear()`,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        }
        try {
          stepTracker?.addStep({
            type: 'action', action: 'clear', selector, status: 'passed',
            description: `page.${desc}.clear()`,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        if (typeof el.focus === 'function') el.focus();
        const localDocWindow = doc.defaultView || doc.parentWindow || window;
        const nativeInputValueSetter = localDocWindow?.HTMLInputElement?.prototype
          ? Object.getOwnPropertyDescriptor(localDocWindow.HTMLInputElement.prototype, 'value')?.set
          : null;
      ...
      fill: async (value) => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'fill', selector, value, status: 'failed',
              error: error.message,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        ...
        if (!('value' in el)) {
          const error = new Error(`${desc}.fill(): element does not support .value`);
          try {
            stepTracker?.addStep({
              type: 'action', action: 'fill', selector, value, status: 'failed',
              error: error.message,
        ...
        if (cursorAnimationCallback && position) {
          try {
            await cursorAnimationCallback({
              action: 'fill', position, selector,
              description: `page.${desc}.fill('${value}')`, value,
            });
          } catch (e) { console.warn('Cursor animation failed:', e); }
        }
        ...
        try {
          stepTracker?.addStep({
            type: 'action', action: 'fill', selector, value, status: 'passed',
            description: `page.${desc}.fill('${value}')`, position,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
      ...
      textContent: async () => {
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'textContent', selector, status: 'passed',
            description: `page.${desc}.textContent()`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      innerText: async () => {
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'innerText', selector, status: 'passed',
            description: `page.${desc}.innerText()`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      isVisible: async () => {
        ...
        if (!el) {
          try {
            stepTracker?.addStep({
              type: 'query', action: 'isVisible', selector, status: 'passed',
              description: `page.${desc}.isVisible()`, result: false,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          return false;
        }
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'isVisible', selector, status: 'passed',
            description: `page.${desc}.isVisible()`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      isChecked: async () => {
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'isChecked', selector, status: 'passed',
            description: `page.${desc}.isChecked()`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      getAttribute: async (attrName) => {
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'getAttribute', selector, status: 'passed',
            description: `page.${desc}.getAttribute('${attrName}')`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      count: async () => {
        ...
        try {
          stepTracker?.addStep({
            type: 'query', action: 'count', selector, status: 'passed',
            description: `page.${desc}.count()`, result,
          });
        } catch (e) { if (e instanceof StopExecutionError) throw e; }
        return result;
      },
      ...
      blur: async () => {
        ...
        try {
          stepTracker?.addStep({
            type: 'action', action: 'blur', selector, status: 'passed',
      ...
      press: async (key) => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'press', selector, status: 'failed',
              error: error.message,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        ...
        try {
          stepTracker?.addStep({
            type: 'action', action: 'press', selector, status: 'passed',
      ...
      selectOption: async (value) => {
        ...
        if (!el) {
          ...
          try {
            stepTracker?.addStep({
              type: 'action', action: 'selectOption', selector, status: 'failed',
              error: error.message,
            });
          } catch (e) { if (e instanceof StopExecutionError) throw e; }
          throw error;
        ...
        try {
          stepTracker?.addStep({
            type: 'action', action: 'selectOption', selector, value, status: 'passed',
        ...
        for (const el of allElements) {
          ...
          if (matched) {
            ...
            for (const child of el.children) {
              ...
              if (isRegex ? text.test(childContent) : exact ? childContent.trim() === text : childContent.includes(text)) {
                childMatches = true;
                break;
              }
            }
  ...
  function expect(actual, stepTracker, negated = false) {
    ...
    const matchers = {
      async toBe(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBe', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBe(): ${errorMsg}`);
      },
      ...
      async toMatch(pattern) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toMatch', expected: String(pattern), actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toMatch(): ${errorMsg}`);
      },
      ...
      async toContain(expectedSubstring) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toContain', expected: expectedSubstring, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toContain(): ${errorMsg}`);
      },
      ...
      async toEqual(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toEqual', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toEqual(): ${errorMsg}`);
      },
      ...
      async toBeGreaterThan(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeGreaterThan', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeGreaterThan(): ${errorMsg}`);
      },
      ...
      async toBeLessThan(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeLessThan', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeLessThan(): ${errorMsg}`);
      },
      ...
      async toBeGreaterThanOrEqual(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeGreaterThanOrEqual', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeGreaterThanOrEqual(): ${errorMsg}`);
      },
      ...
      async toBeLessThanOrEqual(expected) {
        ...
        const errorMsg = negated
          ...
          : `expected ${value} to be <= ${expected}`;
        addStepSafe({
          type: 'assertion', action: 'toBeLessThanOrEqual', expected, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeLessThanOrEqual(): ${errorMsg}`);
      },
      ...
      async toBeNull() {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeNull', expected: null, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeNull(): ${errorMsg}`);
      },
      ...
      async toBeTruthy() {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeTruthy', expected: true, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeTruthy(): ${errorMsg}`);
      },
      ...
      async toBeFalsy() {
        ...
        const errorMsg = negated
          ...
          : `expected ${JSON.stringify(value)} to be falsy`;
        addStepSafe({
          type: 'assertion', action: 'toBeFalsy', expected: false, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeFalsy(): ${errorMsg}`);
      },
      ...
      async toBeUndefined() {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeUndefined', expected: undefined, actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeUndefined(): ${errorMsg}`);
      },
      ...
      async toBeDefined() {
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeDefined', expected: 'defined', actual: value,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toBeDefined(): ${errorMsg}`);
      },
      ...
      async toBeVisible(opts) {
        ...
        while (Date.now() - start < timeout) {
          ...
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toBeVisible', status: 'passed',
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeVisible', status: 'failed',
          description: `expect(${actual.selector})${notPrefix}.toBeVisible()`,
          error: errorMsg,
        });
      ...
      async toContainText(expected) {
        ...
        while (Date.now() - start < timeout) {
          ...
          const contained = lastText != null && (isRegex ? expected.test(lastText) : lastText.includes(expected));
          const passed = negated ? !contained : contained;
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toContainText', expected, actual: lastText,
              status: 'passed',
      ...
      async toHaveText(expected) {
        ...
        while (Date.now() - start < timeout) {
          ...
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toHaveText', expected, actual: lastText,
              status: 'passed',
      ...
      async toHaveCount(expected) {
        ...
        while (Date.now() - start < timeout) {
          ...
          const passed = negated ? lastCount !== expected : lastCount === expected;
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toHaveCount', expected, actual: lastCount,
              status: 'passed',
      ...
      async toBeChecked(opts) {
        ...
        while (Date.now() - start < timeout) {
          ...
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toBeChecked', status: 'passed',
        ...
        addStepSafe({
          type: 'assertion', action: 'toBeChecked', status: 'failed',
          description: `expect(${actual.selector})${notPrefix}.toBeChecked()`,
          error: errorMsg,
        });
      ...
      async toHaveValue(expected) {
        ...
        while (Date.now() - start < timeout) {
          const el = actual._rawGetTargetElement ? actual._rawGetTargetElement() : null;
          lastValue = el ? el.value : undefined;
          const matches = expected instanceof RegExp ? expected.test(lastValue || '') : lastValue === expected;
          const passed = negated ? !matches : matches;
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toHaveValue', expected, actual: lastValue,
              status: 'passed',
      ...
      async toHaveAttribute(name, expected) {
        ...
        while (Date.now() - start < timeout) {
          ...
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toHaveAttribute', expected, actual: lastAttr,
              status: 'passed',
        ...
        throw new Error(`expect(...)${notPrefix}.toHaveAttribute(): ${errorMsg}`);
      },
      ...
      async toHaveURL(expected) {
        ...
        let lastUrl = '';
        while (Date.now() - start < timeout) {
          lastUrl = (actual && actual._getUrl) ? actual._getUrl() : (window.location.pathname + window.location.search);
          // Strip virtual proxy prefix and query params for matching
          var urlPath = lastUrl.replace(/^\/__(virtual|dev)__\/\d+/, '').split('?')[0];
          let matches;
          if (expected instanceof RegExp) {
            matches = expected.test(urlPath) || expected.test(lastUrl);
          } else {
            matches = urlPath === expected || lastUrl === expected || urlPath.endsWith(expected);
          }
          const passed = negated ? !matches : matches;
          if (passed) {
            addStepSafe({
              type: 'assertion', action: 'toHaveURL', expected, actual: lastUrl,
              status: 'passed',
      ...
      async toHaveLength(expected) {
        ...
        addStepSafe({
          type: 'assertion', action: 'toHaveLength', expected, actual: length,
          status: passed ? 'passed' : 'failed', description,
          error: passed ? null : errorMsg,
        });
        if (!passed) throw new Error(`expect(...)${notPrefix}.toHaveLength(): ${errorMsg}`);
      },
    };
  ...
  function buildPageObject(targetDocument, stepTracker, options) {
    return {
      __isPage: true,
      locator: (selector) => createLocator(targetDocument, selector, stepTracker),
      getByTestId: (id) => createLocator(targetDocument, `[data-testid="${id}"]`, stepTracker),
      getByText: (text, opts) => createTextLocator(targetDocument, text, opts, stepTracker),
      getByRole: (role, opts) => createRoleLocator(targetDocument, role, opts, stepTracker),
      getByLabel: (label) => createLabelLocator(targetDocument, label, stepTracker),
      goto: options.onGoto || (async () => {}),
      waitForLoadState: options.onWaitForLoadState || (async () => { await delay(1000); }),
      waitForTimeout: async (ms) => {
        stepTracker.addStep({
          type: 'action', action: 'waitForTimeout', ms, status: 'passed',
          description: `page.waitForTimeout(${ms})`,
        });
        await delay(ms);
      },
      waitForURL: async () => { await delay(500); },
      evaluate: async (fn) => {
        try {
          const docWin = targetDocument.defaultView || targetDocument.parentWindow || window;
      ...
      _getUrl: () => {
        try {
          const docWin = targetDocument.defaultView || targetDocument.parentWindow;
          if (docWin && docWin.location) return docWin.location.pathname + docWin.location.search;
  ...
  async function runTests(options) {
    ...
    if (!targetDocument) throw new Error('runTests: targetDocument is required');
    ...
    for (let testIndex = 0; testIndex < tests.length; testIndex++) {
      ...
      const stepTracker = {
        addStep: (step) => {
          const stepWithIndex = { ...step, timestamp: Date.now(), index: currentStepIndex };
      ...
      onTestBegin(name);
      const page = buildPageObject(targetDocument, stepTracker, { onGoto, onWaitForLoadState });
  ...
  async function runSingleTest(testName, options) {
    ...
    if (!targetDocument) throw new Error('runSingleTest: targetDocument is required');
  ...
  function createContext() {
    ...
    function _test(name, optionsOrFn, fn) {
      ...
      _tests.push({ name: fullName, fn: testFn, device: options.device || 'desktop' });
    }
    ...
    function _clearTests() { _tests.length = 0; }
    function _getTests() { return _tests; }
    function _getTestNames() { return _tests.map(t => t.name); }
    function _getTestsWithMetadata() { return _tests.map(t => ({ name: t.name, device: t.device })); }
    function _setCursorCallback(cb) { _cursorAnimationCallback = cb; }
