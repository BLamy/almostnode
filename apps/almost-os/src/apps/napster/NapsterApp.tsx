...
type HomeTab = "main" | "new-releases" | "staff-picks" | "playlists" | "radio";
...
export function NapsterApp() {
  ...
  const [needsAuth, setNeedsAuth] = useState(!isSignedIn());
  ...
  const loadHome = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof SoundCloudAuthError) setNeedsAuth(true);
    }
  }, []);
  ...
  const runSearch = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof SoundCloudAuthError) {
        setNeedsAuth(true);
        setError(e.message);
      } else {
  ...
  const signIn = () => system.runInTerminal("sc login");
    ...
    <div className="napster">
      ...
      <div className="napster__topbar">
        ...
        <div className="napster__account">
          <span>Welcome{needsAuth ? " guest" : ""}</span>
          <a role="button" tabIndex={0}>My Account</a>
          <a role="button" tabIndex={0}>Help</a>
          <a role="button" tabIndex={0} onClick={needsAuth ? signIn : undefined}>
      ...
      <div className="napster__body">
        ...
        <aside className="napster__sidebar">
          ...
          <div className="napster__panel">
            <div className="napster__panel-head napster__panel-head--collapsed">Radio Stations ▾</div>
          </div>
          <div className="napster__panel">
        ...
        <main className="napster__content">
          ...
          {needsAuth && (
            <div className="napster__auth">
              You're not signed in to SoundCloud — searches and new-release feeds need it.
              <button type="button" onClick={signIn}>Sign in (runs <code>sc login</code>)</button>
            </div>
          )}
          ...
          <div className="napster__scroll">
            ...
            {view === "library" && (
              <LibraryList library={library} onPlay={playFromLibrary} />
            )}
  ...
  // Search from a sidebar term (genre) without waiting on the input state.
  async function searchByTerm(term: string) {
    setView("search");
    setLoading(true);
    setError(null);
    try {
      setResults(await searchTracks(term, { limit: 30 }));
      setNeedsAuth(false);
    } catch (e) {
      if (e instanceof SoundCloudAuthError) setNeedsAuth(true);
      setError(e instanceof Error ? e.message : "Search failed.");
...
function TrackCard({
  ...
  return (
    <div className="napster__card">
      ...
      <button
        type="button"
        className={`napster__dl${downloaded ? " is-done" : ""}`}
  ...
  return (
    <>
      ...
      {newReleases.length === 0 ? (
        <div className="napster__empty">Sign in to load new releases.</div>
      ) : (
        <div className="napster__grid">
          {newReleases.map((t) => (
            <TrackCard
      ...
      <ol className="napster__top10">
        {topTracks.map((t, i) => (
          <li key={t.id} className="napster__top10-row">
            ...
            <button
              type="button"
              className={`napster__dl napster__dl--sm${downloadedUrls.has(t.permalinkUrl) ? " is-done" : ""}`}
              onClick={() => onDownload(t)}
            >
              {downloadedUrls.has(t.permalinkUrl) ? "✓" : "⬇"}
            </button>
          </li>
        ))}
        {topTracks.length === 0 && <div className="napster__empty">Sign in to load charts.</div>}
      </ol>
    </>
  );
}
...
function LibraryList({
  ...
  return (
    <table className="napster__library">
      ...
      <tbody>
        {library.map((e) => (
          <tr key={e.path} onDoubleClick={() => onPlay(e)} title="Double-click to play in Winamp">
