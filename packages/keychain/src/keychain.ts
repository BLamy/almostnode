...
export interface KeychainOptions {
  ...
  /**
   * Predicate that decides whether a terminal command launches an agent
   * harness (claude/codex/opencode/pi …) and should trigger vault
   * auto-restore. Injected so the headless engine stays free of CLI-launch
   * regexes; the demo wires this to its terminal-command-routing matchers.
   * Defaults to never matching.
   */
  isAgentLaunchCommand?: (command: string) => boolean;
}
...
export class Keychain {
  ...
  private readonly isAgentLaunchCommand: (command: string) => boolean;
  ...
  constructor(options: KeychainOptions) {
    ...
    this.isAgentLaunchCommand = options.isAgentLaunchCommand ?? (() => false);
  }
  ...
  async prepareForCommand(command: string): Promise<boolean> {
    ...
    const shouldAutoRestore = this.isAgentLaunchCommand(command)
