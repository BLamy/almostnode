...
import classStaticBlock from "@babel/plugin-transform-class-static-block";
import privateMethods from "@babel/plugin-transform-private-methods";
import privatePropertyInObject from "@babel/plugin-transform-private-property-in-object";
...
const almostnodeSrc = resolve(workspaceRoot, "packages/almostnode/src");
...
const opentuiWasmUrl = `data:application/wasm;base64,${readFileSync(opentuiWasmSourcePath).toString("base64")}`;
const streamBrowserifyPath = require.resolve("stream-browserify");
const processShimPath = require.resolve("vite-plugin-node-polyfills/shims/process");
const globalShimPath = require.resolve("vite-plugin-node-polyfills/shims/global");
...
function downlevelMetroIncompatibleSyntax() {
  return {
    name: "downlevel-metro-incompatible-syntax",
    apply: "build" as const,
    enforce: "post" as const,
    async renderChunk(code: string, chunk: { fileName: string; type: string }) {
      ...
      const result = await transformAsync(code, {
        ...
        plugins: [
          ...
          [privatePropertyInObject, { loose: true }],
        ],
      });
...
export default defineConfig(async ({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      redirectModuleImport(resolve(__dirname, "src/opencode-models-snapshot.ts"), {
      ...
      wasm(),
      downlevelMetroIncompatibleSyntax(),
      nodePolyfills({
        include: [
          "path",
          "util",
    ...
    define: {
      ...
      __OPENTUI_WASM_URL__: JSON.stringify(opentuiWasmUrl),
    },
    resolve: {
      dedupe: ["solid-js"],
      alias: [
        ...
        },
        {
          ...
          replacement: resolve(__dirname, "src/opentui-testing.ts"),
        },
        {
          find: "@opentui/core",
          replacement: resolve(__dirname, "src/opentui-core.ts"),
        },
        {
        ...
        {
          find: /^yoga-layout$/,
          replacement: resolve(__dirname, "src/yoga-layout-shim.ts"),
        },
        {
          find: "@opentui/solid",
