// Tailscale Connect package — ESM wrapper around Go WASM runtime.
// Built for Go 1.25.1 wasm_exec.js runtime.

// ─── Go WASM runtime (wasm_exec.js from Go 1.25.1) ───────────────────────────
...
const defaultWasmURL = "./main.wasm";

/**
 * Create a Tailscale IPN instance by loading the Go WASM binary.
 */
export async function createIPN(config) {
  const go = new Go();
  const wasmURL = config.wasmURL ?? defaultWasmURL;
  const result = await WebAssembly.instantiateStreaming(
    fetch(wasmURL),
    go.importObject,
  );
  go.run(result.instance).then(() => config.panicHandler("Unexpected shutdown"));
