...
interface RegisteredShellCommand {
  name: string;
  trusted: boolean;
...
export interface ChildProcessController {
  ...
  userShellCommands: Map<string, RegisteredShellCommand>;
  activeShellCommands: Map<string, RegisteredShellCommand>;
...
function createRegisteredShellCommand(
  name: string,
  command: JustBashCommand,
  options?: {
    trusted?: boolean;
  ...
  return {
    ...
    interceptShellParsing: !!options?.interceptShellParsing,
    command,
  };
}
...
function unregisterActiveShellCommand(
  ...
  for (const binPath of [`/bin/${name}`, `/usr/bin/${name}`]) {
    if (controller.vfs.existsSync(binPath)) {
      try {
        controller.vfs.unlinkSync(binPath);
      } catch {
        // Ignore cleanup errors for synthetic command shims.
      }
    }
  }
}

function createShellCommandExecutionContext(
  controller: ChildProcessController,
  env: Record<string, string>,
  cwd: string,
  stdin = ''
  ...
  const ctx: CommandContext = {
    fs: controller.vfsAdapter,
    cwd: currentCwd,
    env: envMap,
    stdin,
    signal: getExecutionContextFromEnv(controller, env)?.signal ?? undefined,
    exec: async (command, options) => {
      const resolvedCwd = normalizeCommandCwd(options.cwd || currentCwd);
  ...
  return {
    ctx,
    getEnv: () => {
      const next = stripInternalChildProcessEnv(Object.fromEntries(envMap));
      next.PWD = next.PWD || currentCwd;
      return next;
    },
  };
}
...
function adaptShellCommandDefinition(
  ...
  const command = defineCommand(definition.name, async (args, ctx) => {
    ...
    const execution = getExecutionContextFromEnv(controller, envRecord);
    let currentCwd = normalizeCommandCwd(ctx.cwd);
    ...
    const shellContext: ShellCommandContext = {
      ...
      getEnv: () => ({ ...currentEnv }),
      setCwd: (nextCwd) => {
        const resolved = resolveFromCwd(currentCwd, nextCwd);
        currentEnv.OLDPWD = currentEnv.PWD || currentCwd;
        currentCwd = resolved;
        currentEnv.PWD = resolved;
        syncEnvMap();
      },
      exec: async (command, options = {}) => {
        if (!ctx.exec) {
  ...
  return createRegisteredShellCommand(definition.name, command, {
    ...
    interceptShellParsing: definition.interceptShellParsing,
  });
}
...
function maybeRunCustomCommandDirect(
  ...
  return (async () => {
    ...
    return {
      ...
      stderr: result.stderr,
      exitCode: result.exitCode,
      env: result.env || executionContext.getEnv(),
      stdoutEncoding: result.stdoutEncoding,
    };
  })();
}
...
module.exports = (async () => {
  ...
  // Wrap bashInstance.exec to normalize Unicode curly quotes and intercept
  // registered commands that need to bypass the shell lexer.
  ...
  controller = {
    ...
    registerShellCommand: (definition) => {
      ...
      controller.userShellCommands.set(definition.name, registered);
      registerActiveShellCommand(controller, registered);
    },
    unregisterShellCommand: (name) => {
      const hadUserCommand = controller.userShellCommands.delete(name);
      if (!hadUserCommand) {
        return false;
      }
      const builtin = controller.builtinShellCommands.get(name);
      if (builtin) {
        registerActiveShellCommand(controller, builtin);
      } else {
        unregisterActiveShellCommand(controller, name);
      }
      return true;
    },
  };
  ...
  for (const shellCommand of builtinShellCommands) {
    controller.builtinShellCommands.set(shellCommand.name, shellCommand);
    registerActiveShellCommand(controller, shellCommand);
  }
